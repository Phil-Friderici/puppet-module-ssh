  ...
  describe 'ssh_version' do
    version_matrix.sort.each do |ssh_version_string, result|
      context "with <#{ssh_version_string}> it should return <#{result[:ssh_version]}>" do
        ...
        it do
          Facter.fact(:ssh_version).value.should == "#{result[:ssh_version]}"
        end
      end
    end
  end

  describe 'ssh_version_numeric' do
    version_matrix.sort.each do |ssh_version_string, result|
      context "with <#{result[:ssh_version]}> it should return <#{result[:ssh_version_numeric]}>" do
        before do
          Facter.clear
          allow(Facter.fact(:ssh_version)).to receive(:value).and_return("#{result[:ssh_version]}")
        end

        it do
          Facter.fact(:ssh_version_numeric).value.should == "#{result[:ssh_version_numeric]}"
        end
      end
    end
  end
